defmodule BinanceApi.Order.USDMFuturesTest do
  use ExUnit.Case, async: true

  alias BinanceApi.Order.USDMFutures

  @moduletag :usdm_orders
  ...
  describe "Opening, Viewing and Canceling Orders" do
    ...
    test "orders can be open and then viewed with open orders before being canceled" do
      ...
      assert {:ok, [%{"order_id" => ^order_id}]} = USDMFutures.open_orders([])
      assert {:ok, [%{"order_id" => ^order_id}]} = USDMFutures.open_orders_by_symbol(@symbol, [])
  ...
  def cancel_and_assert_order(order_id) do
    assert {:ok, %{
      "status" => "CANCELED",
      "side" => "BUY"
    }} = USDMFutures.cancel_order(@symbol, order_id, [])
  end
end
